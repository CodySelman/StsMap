...
namespace StsMap
{
    public class MapView : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // TODO delete comment if above singleton setup works as expected
            // Instance = this;
            cam = Camera.main;
        }
        ...
        public void ShowMap(Map m)
        {
            if (m == null)
            {
                Debug.LogError("Map was null in MapView.ShowMap()");
                return;
            }
        ...
        private void CreateMapBackground(Map m)
        {
            ...
            backgroundObject.transform.localPosition = new Vector3(0, span / 2f, 0);
        ...
        public void AddLineConnection(MapNode from, MapNode to)
        {
            ...
            // TODO might be an issue for pixel art
        ...
        {
            var config = GetConfig(mapManager.CurrentMap.configName);
            return config.nodeBlueprints.FirstOrDefault(n => {
                Debug.Log(n.name);
                return n.name == blueprintName;});
        }
    }
}
